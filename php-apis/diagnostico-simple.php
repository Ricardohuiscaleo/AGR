<?php
/**
 * Script de diagnóstico simple para el sistema Agente RAG
 * No usa funciones restringidas como exec()
 */
...
function verificarIntegridadArchivo($ruta) {
    ...
    if (substr_count($contenido, '{') !== substr_count($contenido, '}')) {
        return [
            'estado' => 'error',
            'mensaje' => "El archivo tiene un número desigual de llaves de apertura y cierre"
        ];
    }
    
    return [
        'estado' => 'ok',
        'mensaje' => "El archivo parece estar completo"
    ];
}
