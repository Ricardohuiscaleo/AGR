 ...
 * Replica las tools de n8n: Calendar, Contact, Document, Email
 */
...
class GabyTools {
    ...
    /**
     * CALENDAR TOOL - Gestión de calendario y reuniones
     */
    ...
    /**
     * CONTACT TOOL - Gestión de contactos y leads
     */
    ...
    /**
     * DOCUMENT TOOL - Generación de informes y documentos
     */
...
- Tiempo perdido: " . (isset($data['time_lost']) ? $data['time_lost'] : 'No especificado') . "
- Personas involucradas: " . (isset($data['people_involved']) ? $data['people_involved'] : 'No especificado') . "
...
Genera un informe profesional en HTML con recomendaciones específicas.";
    ...
    /**
     * EMAIL TOOL - Gestión de correos electrónicos
     */
    ...
    private function sendEmail($data) {
        ...
        // Usar mail() de PHP (básico pero funcional)
        $headers = [
...
function createGabyTables() {
    ...
    try {
        ...
        // Tabla de reuniones (actualizada) - Verificar si columna existe antes de agregar
        $result = $pdo->query("SHOW COLUMNS FROM gaby_meetings LIKE 'google_event_id'");
        if ($result->rowCount() == 0) {
            $pdo->exec("ALTER TABLE gaby_meetings ADD COLUMN google_event_id VARCHAR(255)");
        }
